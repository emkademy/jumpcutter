...
class xmlGen:
    ...
    def getDuration(self):
        ...
        for clips in self.cuts:
            ...
            self.durations.append(framCut[1]-framCut[0])
    ...
    def makeXML(self):
        ...
        name=ElementTree.SubElement(seq, "name").text=os.path.splitext(self.clipName)[0]+" Split"
        ...
        at=ElementTree.SubElement(audio, "track")
        for cuts in self.frameCuts:
            self.addVideoTrack(vt,cuts)##probably need to edit last lines to link up more than 1 audio track
            self.addAudioTrack(at,cuts)## only supporting 1 audio track
            self.clipNumber+=1
            self.curFrame+=cuts[1]-cuts[0]
        ElementTree.SubElement(at, "enabled").text="TRUE"
        ElementTree.SubElement(at, "locked").text="FALSE"
    def addAudioTrack(self,vt,cuts):
        clipItem=ElementTree.SubElement(vt, "clipitem",{"id":"{} {}".format(self.clipName,self.clipNumber)})
        ...
        ElementTree.SubElement(clipItem, "stop").text=str(self.curFrame+cuts[1]-cuts[0])
        ...
        ElementTree.SubElement(clipItem, "file",{"id":"{} {}".format(self.clipName,"vid")})
        sourceTrack=ElementTree.SubElement(clipItem,"sourcetrack")
        ElementTree.SubElement(clipItem, "mediatype").text="audio"
        ElementTree.SubElement(clipItem, "trackindex").text="audio"
        link1=ElementTree.SubElement(clipItem, "link")
        ElementTree.SubElement(link1, "linkclipref").text="{} {}".format(self.clipName,self.clipNumber-1)
        link2=ElementTree.SubElement(clipItem, "link")
        ElementTree.SubElement(link2, "linkclipref").text="{} {}".format(self.clipName,self.clipNumber)
    def addVideoTrack(self,vt,cuts):
        clipItem=ElementTree.SubElement(vt, "clipitem",{"id":"{} {}".format(self.clipName,self.clipNumber)})
        ...
        ElementTree.SubElement(clipItem, "in").text=str(cuts[0])
        ElementTree.SubElement(clipItem, "out").text=str(cuts[1])
        file=ElementTree.SubElement(clipItem, "file",{"id":"{} {}".format(self.clipName,"vid")})
        if self.firstClip:
            ElementTree.SubElement(file, "duration").text=str(self.fullClipDurration)
            self.addRate(file)
            ElementTree.SubElement(file, "out").text=self.clipName
            pathToFile="file://localhost/{}".format("/".join(os.path.split(self.clip_path)))
        ...
        link1=ElementTree.SubElement(clipItem, "link")
        ElementTree.SubElement(link1, "linkclipref").text="{} {}".format(self.clipName,self.clipNumber)
        self.clipNumber+=1
